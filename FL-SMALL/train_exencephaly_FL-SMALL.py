...
# -*- coding: utf-8 -*-
"""
Deterministic 3-plane CNN-ViT for binary exencephaly classification
on 144� �CT embryo cubes (Absent vs Present).

Key points
??????????????????????????????????????????????????????????????????
? Focal-Loss (? = 2) and DropPath = 0 are unchanged.
? Capacity trimmed (embed = 128, heads = 4, depth = 4).
? ResNet-50 slice encoder initialised with ImageNet weights.
? **BUG-FIX:** slice encoder now projects to **128 dims** so its output
  matches the embedding layer?s expectation (fixes cat size error).
"""
# ------------------------------------------------------------------- imports
...
import torchvision.models as models          # torchvision ? 0.15
...
class MultiPlane_MultiSlice_Extract_Project(nn.Module):
    """
    2-D ResNet-50 over slice mosaics.
    conv1 is rebuilt for 64-ch input (random init); all other layers
    keep ImageNet-1K weights.  Final projection now outputs `emb_size`
    (128 by default) to match the transformer embedding dimension.
    """
    def __init__(self, out_ch=64, p=0.20, emb_size=128):   # ? emb_size arg
        super().__init__()
        ...
        self.proj = nn.Sequential(nn.Linear(2048, 512), nn.ReLU(),
                                  nn.Linear(512, emb_size))  # ? 128-dim
...
class M3T_Exencephaly(nn.Module):
    """
    Top-level model: e = 128, depth = 4.
    """
    def __init__(self,
                 in_ch=1, out_ch=64, e=128, depth=4,
                 p_cnn3d=0.10, p_feat=0.20, p_tok=0.10,
                 p_trans=0.10, p_dp=0.0):
        super().__init__()
        self.cnn3d = CNN3DBlock(in_ch, out_ch, p_cnn3d)
        self.project = MultiPlane_MultiSlice_Extract_Project(out_ch, p_feat, e)  # ? pass e
...
class FocalLoss(nn.Module):
    """
    Weighted multi-class focal loss with fixed ?.
    """
