...
class MultiPlane_MultiSlice_Extract_Project(nn.Module):
    def __init__(self, out_channels: int, feat_drop_p: float = 0.05):
        ...
        self.proj = nn.Sequential(nn.Linear(2048, 512), nn.ReLU(inplace=True), nn.Linear(512, 256))
...
class FeedForward(nn.Sequential):
    def __init__(self, emb, exp=2, p=0.1):
        super().__init__(nn.Linear(emb,exp*emb), nn.GELU(), nn.Dropout(p), nn.Linear(exp*emb,emb))

class TransformerBlock(nn.Module):
    def __init__(self, emb=256, p=0.1, dp=0.05, exp=2):
        super().__init__()
        self.dp1, self.dp2 = DropPath(dp), DropPath(dp)
        self.res1 = ResidualAdd(nn.Sequential(nn.LayerNorm(emb), MultiHeadAttention(emb,p=p), nn.Dropout(p)))
        self.res2 = ResidualAdd(nn.Sequential(nn.LayerNorm(emb), FeedForward(emb,exp,p), nn.Dropout(p)))
    def forward(self,x):
...
class Encoder(nn.Module):
    def __init__(self, cfg):
        ...
        self.project = MultiPlane_MultiSlice_Extract_Project(cfg['out_channels'], cfg['p_feat'])
        self.embed   = EmbeddingLayer(cfg['emb_size'], total_tokens=432, token_drop_p=cfg['p_token'])
        self.trans   = TransformerEncoder(cfg['depth'], cfg['emb_size'], cfg['p_transformer'], cfg['p_drop_path'])
    def forward(self, x):
        feat3d = self.cnn3d(x)
